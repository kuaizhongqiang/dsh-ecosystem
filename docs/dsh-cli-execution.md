...
- **范围**：`plugin.*` / `skill.*` / `cred.*` / `ecosystem.*` / `runtime.*`；launcher 拉起 `dshcli serve`（复用注册与重启 seam）。
...
- **验收**：launcher 能**安装**、**检查更新**、**升级** dsh-cli 并按需拉起；launcher 启动后 CLI 服务在跑且已注册；插件 / 凭证可查可改。
