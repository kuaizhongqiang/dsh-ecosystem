...
- **聊天面板**（Webview）：
  ...
  - 停止当前回合（输入区与发送并排）、一键在浏览器打开同一会话、底部用量统计条（轮次 / 步数 / tokens / 上下文占用 / **累计费用 ¥**，内置 DeepSeek 官方**峰谷 + 调价历史**价目表与小米 MiMo 官方价，按会话最早事件时间取档；可在 `dsh.pricing` 设置覆盖）
...
| `dsh.pricing` | 内置官方价 | 用量栏费用估算的价格表：按模型 id 的每百万 tokens 单价（¥）。默认内置 DeepSeek 官方**价目历史**（2026-08-17 00:00 起峰谷定价、2026-09-10 12:00 起 V4.1 Flash 降价；高峰 = 北京时间**周一至周五** 9-12、14-18，其余含周末为空闲）与小米 MiMo 价，官方调价后可覆盖 |
...
- 费用按**当前会话模型**的官方价估算累计值，且整段按**会话最早事件时间**取峰谷档与调价生效期（同一会话跨 12:00/18:00 或跨调价日时不逐时分摊；会话内混用多模型时也不能精确分摊到各模型，可在 `dsh.pricing` 覆盖价格）。
