// scripts/verify-pm4.mjs —— PM4 自动验证(skills 11→7;后续追加 install-ue-mcp / install-memory / install-image → 10 + install-skills 联动)。
//
// 覆盖:
//   1. skills 目录恰为 10 个集合(install-media/deepseek/launcher/credentials/github/stock/unity-mcp/ue-mcp/memory/image)
//   2. install-skills.ps1 -DryRun 自动发现 10 个且不含旧名
...
async function main() {
  ...
  const EXPECT = ['install-media', 'install-deepseek', 'install-launcher', 'install-credentials', 'install-github', 'install-stock', 'install-unity-mcp', 'install-ue-mcp', 'install-memory', 'install-image'].sort();
  ...
  console.log('1. skills 目录 = 10 个集合');
  {
    ...
    ok(JSON.stringify(dirs) === JSON.stringify(EXPECT), `1-1 目录恰为 10(${dirs.join(', ')})`);
  }
  ...
  console.log('2. install-skills.ps1 自动发现(-DryRun)');
  {
    ...
    ok(EXPECT.every((s) => out.includes(s)), '2-2 发现全部 10 个');
  ...
  console.log('3. 真实安装到临时 DSH_HOME');
  {
    ...
    ok(JSON.stringify(got) === JSON.stringify(EXPECT), '3-2 %DSH_HOME%\\skills 落位 10 个');
  }
  ...
  console.log('4. 源码级一致性');
  {
    ...
    ok(readme.includes('dsh-image-dsh-plugin') && spec.includes('dsh-image'), '4-3 README 与规范覆盖图片生成层(dsh-image)');
  }
