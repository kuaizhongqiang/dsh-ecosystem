...
旧单工具包已 DEPRECATED:跑仓库根 `uninstall-old.ps1` 清理旧载荷、patch 节**与旧技能**
(技能清理已**默认执行**,要保留旧技能加 `-KeepSkills`)。迁移后应重启 web。

**装本包与跑迁移这两种顺序都安全**:新包与旧包**共用同名载荷目录**(`plugins\audio-read` 等),只有
patch 节头不同,脚本会识别「已被新包接管的载荷」并保留它,不会把刚装好的新包打瘫(见 issue #34)。
