...
板块只分四块、不存在第五个主题：**电力设备**（含 运营/防御、设备/基建 两个子层）与
**航天卫星** 为可交易块；**券商**、**石油** 只做信号、**不建头寸**。主题不符的标的一律移出
股池（已移出航空整机 3 只、国网英大、锂电新能源 5 只——后者 29.9% 的成交额权重会把电力设备
块级读数从"出逃"扭成"主升"，污染信号），当前股池 56 只。

| 工具 | 作用 |
|---|---|
| `sector_panel` | 四块面板：1/5/20 日收益、相对沪深300 超额（RS20）、成交额占比及其量能倍数、上涨家数、β、四象限（主升/抱团/出逃/冷落）；**龙头背离**（龙头取近 20 日成交额最大成员，比较其 5 日相对强度与量能：放量而龙头不涨=资金分散/出货，缩量而龙头独强=存量抱团）；**资金水位**（全池成交额 vs 20 日均值 → 纯存量博弈还是可能有增量）；大盘状态机（牛/震荡/熊）；券商牛市启动信号（默认判假，需连续 2 日 + 大盘站上 MA20 才确认）；石油牛市收尾信号（仅牛市监听）；并给出每块/子层的挂单深度规则 |
| `order_calibration` | 挂单成交率标定：买单挂 `收盘−k×ATR`、卖单挂 `收盘+u×ATR` 的次日成交率、成交后 5 日表现，**以及未成交那一侧的 5 日表现**（买单未成交=踏空成本，卖单未成交=继续扛跌），按趋势分层。先定目标成交率再反查 k 值（建仓 60~70% → k≈0.3；了结 85%+ → u≤0.2） |
