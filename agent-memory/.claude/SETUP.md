...
> 本项目实际采用 **mcp-bridge + Stop hook 自动入库**（见 [CLAUDE.md](../CLAUDE.md)）。
...
# Recall（指定 sender 过滤 = 仅限 crossDomainRecall 的 sender）
curl -X POST https://your-server.com/api/v1/recall \
  -H "Authorization: Bearer sk-your-api-key" \
  -H "X-Sender: openclaw" \
  -H "Content-Type: application/json" \
  -d '{"query": "测试查询", "sender": "openclaw"}'

# Recall（全量搜索，仅限有跨域权限的 sender）
curl -X POST https://your-server.com/api/v1/recall \
  -H "Authorization: Bearer sk-your-api-key" \
  -H "X-Sender: openclaw" \
  -H "Content-Type: application/json" \
  -d '{"query": "测试查询", "senders": ["*"]}'
