...
export async function buildExe() {
  ...
  const exe = join(dist, process.platform === 'win32' ? 'dshcli.exe' : 'dshcli')
  ...
  const versioned = join(dist, process.platform === 'win32' ? `dshcli-${pkg.version}.exe` : `dshcli-${pkg.version}`)
  copyFileSync(exe, versioned)
  return { exe, versioned, version: pkg.version, bytes: readFileSync(exe).length }
}
